'''
def hedge_bets(guess, conf, mean):
    diff = abs(conf[0] - conf[1]) ## this is between 0 (no confidence) and 1 (high confidence)
...
def rms(classifier, data, ex, actual):
    ...
    for (x, d, y, conf) in zip(predictions, ex, actual, probs):
        c = d[2].replace('category', '')
        diff = abs(conf[1] - conf[0])
        z, y = catpos[x][c]*diff + 39.298062750052644*(1-diff), float(y)
        squareSum += (z-y)*(z-y)
        n += 1.0
...
if __name__ == "__main__":
    ...
    o.writeheader()
    for ii, pp, ee, conf in zip([x['id'] for x in test], predictions, examp, probs):
        c = ee[2].replace('category', '')
        diff = abs(conf[1] - conf[0])
        pp = catpos[pp][c]*diff + 39.298062750052644*(1-diff)
        d = {'id': ii, 'position': pp}
        o.writerow(d)
